...
$AZ_PRIVATE_IP_CONTAINER_GROUP=(Get-AzContainerGroup `
  -ResourceGroupName ${AZ_RESOURCE_GROUP_NAME} `
  -Name ${AZ_CONTAINER_GROUP_NAME})

$AZ_PRIVATE_IP_CONTAINER_GROUP=$AZ_PRIVATE_IP_CONTAINER_GROUP.IpAddress
...
$AZ_UNHEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY=(az network application-gateway show-backend-health `
--resource-group $AZ_RESOURCE_GROUP_NAME  `
--name $AZ_APP_GATEWAY_NAME  `
--query "backendAddressPools[].backendHttpSettingsCollection[].servers[?health=='Unhealthy'].address" `
--output tsv)

if ([string]::IsNullOrWhiteSpace($AZ_UNHEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY)) {
  echo "App GW backend pools are healthy."
  echo "Abort."
} else {
  echo "Unhealthy APP GW backend pool servers detected: $AZ_UNHEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY "

  $TMP = ""
  $BAD_SRVS = $AZ_UNHEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY
  $AZ_SERVERS_BACKEND_POOL_APP_GATEWAY.Split(" ") | ForEach {
    $SRVi = $_
    echo "Check if server $SRVi is healthy (NOT in list of unhealthy ones: $BAD_SRVS)."
    if (-Not ($BAD_SRVS -Match $SRVi)) {
      $TMP  = -join($TMP, " ", $SRVi)
      echo "Server $SRVi is healthy. Appended to list: $TMP"
    }
  }
  $AZ_HEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY=$TMP

  echo "New backend pool servers will be: $AZ_HEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY"

  az network application-gateway address-pool update `
  --resource-group ${AZ_RESOURCE_GROUP_NAME} `
  --gateway-name ${AZ_APP_GATEWAY_NAME}  `
  --name ${AZ_BACKEND_POOL_APP_GATEWAY_NAME} `
  --servers $AZ_HEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY.trim().split(" ")
  ...
  echo "New backend pool servers updated to: ${AZ_HEALTHY_SERVERS_BACKEND_POOL_APP_GATEWAY}"
}
