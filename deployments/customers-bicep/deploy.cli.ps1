...
#### Begin STAGE 0: ARG
#################################################################################"
$resourceGroupSuffix = "492233" # "0".PadLeft(3,'0')   # This name is used to generate names for Azure resources, such as storage account name.
$customersChunkSuffix = "0".PadLeft(3,'0')
...
$filesBicep = @(Get-ChildItem -Path bicep-templates -Filter *.bicep -Recurse -Name)

mkdir -p arm-templates

$filesBicep.ForEach({
    $app = 'bicep'
    $appArgs = 'build', "bicep-templates/$_", "--outdir", "arm-templates"
    Write-Output "$app $appArgs"
    Execute "$app $appArgs"
})

$files = @(Get-ChildItem -Path arm-templates -Filter *.json -Recurse -Name)

$mainFileName  = "azuredeploy.json"
...
$targetResourceGroup = Get-AzResourceGroup `
    -Name "myResourceGroup-${resourceGroupSuffix}" `
    -Location ${location} `
    -ErrorVariable deploymentError `
    -ErrorAction SilentlyContinue 

if (-not $targetResourceGroup -or $deploymentError) {
    $targetResourceGroup = New-AzResourceGroup -Name "myResourceGroup-${resourceGroupSuffix}"  -Location ${location}
}
...
$Validation = Test-AzResourceGroupDeployment `
    ...
    -customersChunkSuffix $customersChunkSuffix `
    -location "eastus" `
